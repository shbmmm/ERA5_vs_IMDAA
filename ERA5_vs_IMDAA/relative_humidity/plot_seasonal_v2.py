...
# Load the data
imdaa = xr.open_dataset('imdaa_rh_2023_seasmean.nc')
era5 = xr.open_dataset('era5_rh_2023_regridded_time_v2_seasmean.nc')
difference = xr.open_dataset('diff_seasmean.nc')

# Extract the temperature variables
temp_imdaa = imdaa['RH_2m']
temp_era5 = era5['r']
temp_diff = difference['RH_2m']
...
def plot_subplot(ax, data, title, cbar_label, cmap, cbar_lim=None, discrete=False):
    if discrete:
        levels = np.arange(cbar_lim[0], cbar_lim[1]+ .1, 1)
...
for i in range(4):
    ...
    ax1 = axes[i, 0]
    plot_subplot(ax1, temp_imdaa.sel(time=temp_imdaa.time.isel(time=i)), f'IMD-AA {time_labels[i]}', 'relative_humidity [%]', 'jet', cbar_lim=(20, 80), discrete=True)

    # Plot ERA5 with discretization and specific colorbar limits
    ax2 = axes[i, 1]
    plot_subplot(ax2, temp_era5.sel(time=temp_era5.time.isel(time=i)), f'ERA5 {time_labels[i]}', 'relative_humidity [%]', 'jet', cbar_lim=(20, 80), discrete=True)
