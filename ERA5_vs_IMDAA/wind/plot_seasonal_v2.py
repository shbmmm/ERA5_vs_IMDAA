...
# Load the data
imdaa = xr.open_dataset('imdaa_wind_2023_seasmean.nc')
era5 = xr.open_dataset('era5_wind_regridded_time_v2_seasmean.nc')
difference = xr.open_dataset('diff_seasmean.nc')

# Extract the temperature variables
temp_imdaa = imdaa['UGRD_10m']
temp_era5 = era5['u']
temp_diff = difference['UGRD_10m']
...
def plot_subplot(ax, data, title, cbar_label, cmap, cbar_lim=None, discrete=False):
    if discrete:
        levels = np.arange(cbar_lim[0], cbar_lim[1]+1, 60)
        if len(levels) < 2:
            levels = np.linspace(cbar_lim[0], cbar_lim[1], 50)
...
for i in range(4):
    ...
    ax1 = axes[i, 0]
    plot_subplot(ax1, temp_imdaa.sel(time=temp_imdaa.time.isel(time=i)), f'IMD-AA {time_labels[i]}', 'Temperature [°C]', 'jet', cbar_lim=(0, 10), discrete=True)

    # Plot ERA5 with discretization and specific colorbar limits
    ax2 = axes[i, 1]
    plot_subplot(ax2, temp_era5.sel(time=temp_era5.time.isel(time=i)), f'ERA5 {time_labels[i]}', 'Temperature [°C]', 'jet', cbar_lim=(0, 10), discrete=True)

    # Plot Difference without specific colorbar limits
    ax3 = axes[i, 2]
    plot_subplot(ax3, temp_diff.sel(time=temp_diff.time.isel(time=i)), f'Difference {time_labels[i]}', 'Temperature Difference [°C]', 'coolwarm')

# Add main title and adjust layout
plt.suptitle('Seasonal wind Comparison (IMD-AA vs ERA5)', fontsize=16, y=0.95)
plt.tight_layout()
plt.subplots_adjust(top=0.92)  # Adjust top to fit the suptitle

# Save the figure
plt.savefig('seasonal_wind_comparison.png', dpi=150)
